...
CIoLoop_Epoll::CIoLoop_Epoll()
{
	...
	if (mp_EpollFd == -1)
	{
		...
		DMibPDebugBreak; // Failed to create epoll
	}
...

	{
		...
		for (auto &Change : Changes)
		{
			...
			if (Change.m_bInternal)
			{
				...
				Ev.events = EPOLLIN;
				// The wake pipe is the one registration with no registration behind it, which
				// is how its events are told apart: a registration pointer's low half could
				// equal any descriptor number, a null pointer never is one
				Ev.data.ptr = nullptr;
