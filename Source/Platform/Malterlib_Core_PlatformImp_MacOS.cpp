...
bool NSys::NFile::fg_TryCloneData(const NMib::NStr::CStr &_FileFrom, const NMib::NStr::CStr &_FileTo)
{
	...
	// arrives, and the regular file check below is what rejects it
	...
	// Only a file this attempt cloned is ever removed: the ownership check below proves the
	// directory is this user's, not that this attempt made it, since a directory of this user's
	// that another writer of the destination could rename into place — a leftover of a crashed
	// copy, say — would pass it with its own "File" inside
	...
	// The directory this descriptor holds is the one the mkdir above made: owned by this user
	// and closed to everyone else. Ownership alone does not say so — another writer of the
	// destination can rename a directory of this user's own into the staging name, and a
	// directory that came from somewhere else can carry group or other permissions, which would
	// expose the clone below for the whole strip window
	{
