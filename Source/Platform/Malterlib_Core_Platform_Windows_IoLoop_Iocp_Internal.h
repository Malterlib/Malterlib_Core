...
// The Windows io subsystem: the native entry points and this platform's io knobs, decided once
// in the constructor. The loops and the socket context cache a pointer to it and read plain
// members through the accessors, which without the io debugging overrides answer their compile
// time defaults as constants so the consulting branches fold away
struct CIoSubSystem_Windows : NMib::NSys::CIoSubSystem
{
	CIoSubSystem_Windows();

	bool f_CompletionEnabled() const
	{
#if DMibConfig_IoDebug_Enable
	...
	bool f_SkipSuccessEnabled() const
	{
#if DMibConfig_IoDebug_Enable
	...
	bool f_LoopbackFastPathEnabled() const
	{
#if DMibConfig_IoDebug_Enable
	...
	bool f_DirectSendEnabled() const
	{
#if DMibConfig_IoDebug_Enable
	...
	umint f_RecvBufferBytesOverride() const
	{
#if DMibConfig_IoDebug_Enable
