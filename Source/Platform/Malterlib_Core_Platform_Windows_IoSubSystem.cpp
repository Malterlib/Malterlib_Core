...
CIoSubSystem_Windows::CIoSubSystem_Windows()
{
	...
	m_bSkipSuccessEnabled = fsp_EnvFlag("MalterlibIocpSkipSuccess", true);
...
{
#if DMibConfig_IoDebug_Enable
	// The dumps read the ring, and the base destructor would run them after it is gone; dumping
	// here leaves the base nothing to do. Later traces print directly
	f_DumpStats();

	if (m_pTraceRing)
	{
		m_TraceMode = 1;
		NMemory::CAllocator_NonTrackedHeap::f_Free(m_pTraceRing, sizeof(CIocpTraceEntry) * gc_IocpTraceRingEntries);
		m_pTraceRing = nullptr;
	}
#endif
}
