...
// overlapped send finishes only when the peer acknowledges the bytes. Reporting m_fOnComplete
// that late would serialize every message on a round trip (measured: the last segment of each
// also waits out the peer's delayed acknowledgement), so for these registrations the loop
// reports m_fOnComplete as soon as WSASend accepts the send — like the queued result of a Linux
// zero copy send — and the completion packet runs only m_fOnBufferReleased.
// f_SendReleaseIsPrompt answers false for them. Carried in the options so it is in place before
// the loop can see the registration
...
CIoLoop_Iocp::~CIoLoop_Iocp()
{
	// Nothing is on its way out and nothing was left for the kernel: every io object was closed
	// and acknowledged by the driving thread before it exited, see ~CIoLoop_Base
...
bool CIoLoop_Iocp::fp_Associate(CIocpRegistration *_pRegistration, int &o_Error)
{
	...
	//
	// An inherited handle is asked for it whatever MalterlibIocpSkipSuccess says: the mode sticks
	// to the file object and no call takes it off again, so a creator that set it has set it for
	// this loop too. Recorded as off it would leave a transfer that completes at issue waiting
	// for a packet the handle no longer posts, where asking for it makes the record true. The
	// switch stays a switch for every handle this process created itself
...
umint CIoLoop_Iocp::fp_IterateTimeout(bool _bBlock, DWORD _TimeoutMs)
{
	...
	// would sleep on top of a result it already holds. A receive among them stages its bytes
	// rather than delivering them, and a staged segment counts as nothing reported, so the
	// flush comes before the decision too: parked on top of it, the bytes would wait for
	// whatever wakes the loop next
