...
bool CIoLoop_Iocp::fp_Associate(CIocpRegistration *_pRegistration, int &o_Error)
{
	...
	if (bWantSkipSuccess && hBase == (HANDLE)Socket)
	{
		...
		// A unix socket keeps its whole transfer on the posting thread when the port is skipped, which serializes the
		// receives that carry its bandwidth. An inherited handle takes it regardless: the mode cannot be removed later
		if
		(
			getsockopt(Socket, SOL_SOCKET, SO_PROTOCOL_INFOW, (char *)&ProtocolInfo, &nProtocolInfo) == 0
			&& (ProtocolInfo.dwServiceFlags1 & XP1_IFS_HANDLES)
			&& (ProtocolInfo.iAddressFamily != AF_UNIX || _pRegistration->m_Options.m_bInheritedHandle)
		)
		{
