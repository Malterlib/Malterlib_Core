...
bool CIoLoop_IoUring::fp_StartStream(CUringRegistration *_pRegistration, NStorage::TCSharedPointer<NSys::CIoStreamBackpressure> &&_pBackpressure, umint &_nReported)
{
	...
	// Incremental consumption, which the capability probe required: the kernel fills each
	// buffer across several receives, so what a consumer keeps is charged by the buffer and
	// stays within the bytes it holds plus a partly used buffer at either end
	CIoUringBufReg Reg;
	...
	if (pRing->m_pBackpressure && pRing->m_pBackpressure->m_nLimitBytes)
	{
		...
		// The ring fills one buffer at a time, so what the consumer holds pins at most the two
		// partly used buffers at either end of it beyond its own bytes. The window keeps one
		// buffer more than that, and the resume threshold sits at the hold: parked with only
		// the hold outstanding, the releases of everything else have to bring the count under it
...
bool CIoLoop_IoUring::fp_RefillBid(CUringRegistration *_pRegistration, uint16 _Bid)
{
	...
	if (Backpressure && Backpressure->m_nLimitBytes)
	{
		if (Backpressure->m_nOutstandingBytes.f_Load(NAtomic::gc_MemoryOrder_Acquire) >= Backpressure->m_nLimitBytes)
		{
			...
			// have claimed the flag — either way this bid proceeds. The flag comes down only
			// when nothing is parked: bids parked earlier are brought back by the release that
			// claims it, and a flag taken down under them would leave them parked for good,
			// the ring shrinking bid by bid until a dry ring ends the receive with nothing to
			// resume it. A claimed resume that finds nothing parked is harmless
...
// registration lists it at the back, where this walk reaches it, and a sink that reaches
// fp_PlaceBatch, whose full queue reaps and flushes in turn, only empties the list sooner
