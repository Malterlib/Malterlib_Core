...
int CIoUringRing::f_Submit(uint32 _nMinComplete, bool _bGetEvents)
{
	...
	if (m_bSqPoll)
	{
		...
		// idle decay, or for waiting on completions. The tail's release store and the flag's
		// acquire load order nothing between themselves: without a full fence the poller can
		// see the old tail and go to sleep while this side sees the flag from before it did,
		// and the request sits unprocessed under a wait for its completion
		m_nPendingSubmit = 0;
			...
			[&]() -> bool
			{
				...
				if (fs_Register(Probe.m_RingFd, gc_IoUringRegister_Probe, &s_ProbeResult, 64) == 0)
				{
					...
					if (bStream)
					{
						...
						// actual ring registers — not just that the kernel is new enough. The
						// ring is registered with incremental consumption (6.12), and a kernel
						// that rejects it keeps receives on readiness: without it the kernel
						// retires a whole buffer on every receive however short, so a consumer
						// keeping segments while it assembles a message would pin capacity far
						// beyond the bytes it holds, more than the stream's window can bound
