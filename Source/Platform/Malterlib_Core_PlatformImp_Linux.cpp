...
inline_never CMibCodeAddress NSys::fg_System_GetStackTrace(aint _iDepth)
{
	...
	umint nReturned = fg_System_GetStackTrace(StackTraces, _iDepth - 1);
...
namespace
{
	...
	TCVector<tf_CStr> fg_GetCommandLineParams()
	{
		...
		tf_CStr CmdlinePath = typename tf_CStr::CFormat("/proc/{}/cmdline") << (umint)getpid();
...
{
	// The raw syscall rather than the libc wrapper: the memory manager seeds itself before the
	// lazily resolved libc entry points exist, and a fallback to opening /dev/urandom at that point
	// costs a file open per seed. The syscall exists since Linux 3.17; older kernels answer ENOSYS
	while (_nBytes)
	{
...
bool NSys::fg_HW_GetVirtualMachineInfo(CVirtualMachineInfo& _Info)
{
	...
	try
	{
		if (NMib::NFile::CFile::fs_FileExists(CStrNonTracked("/sys/class/dmi/id/sys_vendor")))
		{
			...
			while (*pVendorLookup)
			{
				umint NameLen = fg_StrLen(*pVendorLookup);
	...
	try
	{
		if (NMib::NFile::CFile::fs_FileExists(CStrNonTracked("/sys/devices/virtual/dmi/id/product_name")))
		{
			...
			{
				umint NameLen = fg_StrLen(*pProductLookup);
...
namespace NMib
{
	umint align_cacheline g_SystemMemory[sizeof(CSystemLinux) / sizeof(umint)];
	...
	namespace NSys
	{
		...
		#pragma clang diagnostic ignored "-Wdeprecated-declarations"
			void fg_InitBaseModuleAddress()
			{
				...
				g_MainModuleBase = (umint)((uint8 const *)FirstHeader.dlpi_phdr - FirstHeader.dlpi_phdr->p_offset);
...
void NSys::fg_CreateSystem()
{
	...
	static_assert(alignof(CSystemLinux) <= umint(DMibPMemoryCacheLineSize), "Aligment error");
...
{
	CStrNonTracked StatusPath = CStrNonTracked::CFormat("/proc/{}/status") << (umint)getpid();
...
namespace NMib
{
	namespace NSys
	{
		namespace NFile
		{
			...
			{
				tf_CStr ExePath = typename tf_CStr::CFormat("/proc/{}/exe") << (umint)getpid();
...
{
	return CStr::CFormat("/proc/{}/exe") << (umint)getpid();
}
...
NMib::NStr::CStr NSys::NFile::fg_GetModulePath(void *_pCode)
{
	...
	{
		if ((umint)ModuleInfo.dli_fbase == g_MainModuleBase)
...
{
	// Opened without blocking: a FIFO with no writer would otherwise hold the open until one
	// arrives, and the regular file check below is what rejects it. A regular file reads the
	// same either way, and the clone and the attribute copy work on the descriptor as it is
...
extern "C" int __sched_cpucount(size_t _SetSize, cpu_set_t const *_pSet)
{
	...
	for (; pSetIterator < pSetIteratorEnd; ++pSetIterator)
	{
		...
		for (umint iBit = 0; iBit < sizeof(Mask) * 8; ++iBit)
		{
