...
void NMib::NStr::NPlatform::fg_SystemDecodeAnsiStr(NMib::NStr::CStrNonTracked const &_In, NMib::NStr::CStrNonTracked &_Out)
{
...
void NMib::NStr::NPlatform::fg_SystemDecodeCodePageStr(NMib::NStr::CStrNonTracked const &_In, NMib::NStr::CStrNonTracked &_Out, uint32 _CodePage)
{
	if (_CodePage != 1252)
		DMibError(NMib::NStr::CStrNonTracked::CFormat("Codepage {} not supported") << _CodePage);
...
void NMib::NStr::NPlatform::fg_SystemDecodeCodePageStr(NMib::NStr::CStr const &_In, NMib::NStr::CStr &_Out, uint32 _CodePage)
{
	if (_CodePage != 1252)
		DMibError(NMib::NStr::CStrNonTracked::CFormat("Codepage {} not supported") << _CodePage);
	fg_SystemDecodeAnsiStr(_In, _Out);
...
bool NSys::fg_System_GetOperatingSystemVersion(int &o_Major, int &o_Minor, int &o_Fix, NProcess::EOperatingSystemArch &o_Arch, bool _bForceUpdate)
{
	if (g_OperatingSystemMajor >= 0 && !_bForceUpdate)
	{
	...
	o_Arch = g_OperatingSystemArch = NProcess::EOperatingSystemArch_le32;
