...
bool CIoLoop_Iocp::fp_StartStream(CIocpRegistration *_pRegistration, NStorage::TCSharedPointer<NSys::CIoStreamBackpressure> &&_pBackpressure)
{
	...
	if (pBackpressure && pBackpressure->m_nLimitBytes)
	{
		...
		// What the consumer holds spans every buffer being filled while it arrives: with nDepth
		// receives in flight its bytes are spread over that many buffers, each of which can also
		// carry the tail of what came before it and the head of what follows, so the hold pins
		// up to twice that many partly used buffers beyond its own bytes. The window keeps one
		// buffer more than that, and the resume threshold sits at the hold: parked with only the
		// hold outstanding, the releases of everything else have to bring the count under it
...
NStorage::TCSharedPointer<CIocpStreamBuffer> CIoLoop_Iocp::fp_TakeStreamBuffer(CIocpRegistration *_pRegistration)
{
	...
	if (Backpressure && Backpressure->m_nLimitBytes)
	{
		if (Backpressure->m_nOutstandingBytes.f_Load(NAtomic::gc_MemoryOrder_Acquire) >= Backpressure->m_nLimitBytes)
		{
			...
			// have claimed the flag — either way this post proceeds. The flag comes down only
			// when no post was refused before: a refused post waits for the resume that only a
			// claim of the flag schedules, and a flag taken down under it would leave the
			// stream short a receive for good. A claimed resume that finds nothing parked is
			// harmless
...
bool CIoLoop_Iocp::fp_PostRecv(CIocpRegistration *_pRegistration, CIocpRecvOp &_Op)
{
	...
	// Taken before anything else runs: the trace below writes to the console, which leaves a
	// last error of its own behind, and a pending receive read back as a failure would be
	// completed here while the kernel still owns its buffer
...
// Bounded per call: a re-post that completes at issue puts its operation back at the tail,
// complete, and a socket that always has data would keep this loop going on its own, past
// the per pass bound of the inline drain, which counts registrations. What is left complete
// at the bound is reported in the next pass, through the inline list
void CIoLoop_Iocp::fp_ReportCompletedRecvs(CIocpRegistration *_pRegistration, umint &_nReported)
{
	...
	while (_pRegistration->m_pRecvHead && _pRegistration->m_pRecvHead->m_bCompleted)
	{
		...
		// head, so the buffer is shared safely, and it takes receives until it is full: a
		// buffer retired with an unused tail would charge a consumer keeping its segments
		// for bytes it never delivered, beyond the partly used buffer at either end of a
		// message that the stream's window allows for. The full buffer retires — consumers
		// keep it alive — and the next post takes a fresh one
...
// Delivers a registration's merged, undelivered segment, if one stands, and takes the
// registration off the flush list with it
...
// The end-of-pass flush, from the front until the list is empty: a flush that restages a
// registration lists it at the back, where this walk reaches it
