...
public:
	...
	NMib::NAggregate::TCAggregate<CPOSIXSocketContext, 64> m_SocketContext;
	...
	NMib::NAggregate::TCAggregate<NMib::NOSXRuntime::CFileChangeNoticationContext, 64> m_FileChangeNoticationContext;
...
inline_never assure_used bint NSys::fg_Compiler_MakeActive(const void *_Reference)
{
	(void)_Reference;
	return true;
}
...
//NAggregate::TCAggregate<NThread::TCThreadLocal<zint32, NOSXDebug::CAllocator_NonTrackedHeap, NThread::EThreadLocalFlag_None>> g_DisableHeapOverride;
...
void NSys::fg_Message(const ch8 *_pMessageType, const ch8 *_pToOutput)
{
	...
	bint bCopy = !fg_StrCmpNoCase(_pMessageType, "Copy");
...
namespace NMib
{
	namespace NSys
	{
		...
		bint fg_System_GetOperatingSystemVersion(int& _oMajor, int& _oMinor, int& _oFix, EOperatingSystemArch& _Arch)
		{
...
static int fg_CopyOrRename(const NMib::NStr::CStr &_FileFrom, const NMib::NStr::CStr &_FileTo, bint _bRename)
{
	CStr From = _FileFrom;
	CStr To = _FileTo;
...
static int fg_CopyOrRename(const NMib::NStr::CStr &_FileFrom, const NMib::NStr::CStr &_FileTo, NMib::NFile::CFileProgress& _Progress, bint _bRename)
{
	CStr From = _FileFrom;
	CStr To = _FileTo;
...
{
	DMibSafeCheck(_Address != nullptr, "Address is null!");
	return fg_GetLocalSys()->m_SocketContext->f_Listen(*(CPOSIXAddress*)_Address, fg_Move(_OnStateChange));
}
...
{
	DMibSafeCheck(_Address != nullptr, "Address is null!");
	return fg_GetLocalSys()->m_SocketContext->f_ListenDatagram(*(CPOSIXAddress*)_Address, fg_Move(_OnStateChange));
}
