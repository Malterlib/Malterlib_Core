...
void NSys::NNetwork::fg_SetSendWindow(void *_pSocket, umint _nBytes, bool _bConfigured)
{
	...
	)
	{
		...
		// A unix socket whose peer has already closed refuses the sizing with EINVAL: the
		// kernel has taken the pair apart and reserves nothing for it any more. Such a socket
		// is about to report its close, so the window it would have had is moot; an accepted
		// socket reaches here in that state when the client hung up during the accept
...
// The family of an open descriptor, as the address type the socket carries; none where the
...
CPOSIXSocket* CPOSIXSocketContext::f_InheritHandle2(void* _pOSSocket, NMib::NFunction::TCFunctionMovable<void (NMib::NNetwork::ENetTCPState _StateAdded)> &&_fOnStateChange)
{
	...
	// The handle comes without the address that would say what it is, and the send window's
	// options are family specific — TCP_NOTSENT_LOWAT does not exist for a unix socket and
	// fails on one. The descriptor answers for itself
	if (pSocket)
