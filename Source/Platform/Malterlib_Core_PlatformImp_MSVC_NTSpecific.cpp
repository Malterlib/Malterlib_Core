	...
	NMib::NStr::CStr fp_GetObjectInfo(HANDLE hObject, NLocalWindows::OBJECT_INFORMATION_CLASS objInfoClass, CProcessingInfo &_ProcessInfo)
	{
		if (m_fNTQueryObject)
		{
			...
			if((ntReturn >= STATUS_SUCCESS) && (pObjectInfo->Buffer != nullptr))
			{
				...
				return fg_ConvertFromWindowsPath(TempStr);
			}
		}
...
HRESULT fg_PatchIAT(HMODULE _hMod, PSTR _pImportedModuleName, PSTR _pImportedProcName, PVOID _pHookingProc, PVOID *_pOriginalProc)
{
	...
	for (uiIter = 0; pImportDescriptor[uiIter].Characteristics != 0; uiIter++)
	{
		...
		if (!fg_StrCmpNoCase(psDLLName, _pImportedModuleName))
		{
			...
			for (; pOriginalFirstThunkIter->u1.Function != NULL; pOriginalFirstThunkIter++, pFirstThunkIter++)
			{
				...
				if (!fg_StrCmpNoCase(pImportByName->Name, _pImportedProcName))
				{
					DWORD dwDummy;
...
HMODULE fg_GetModuleForProcess(DWORD _PID, const WCHAR *szModNameArg)
{
	...
	if (EnumProcessModules(hProcess, ahmModules, sizeof(ahmModules), &dwBytes))
	{
		for (uiIter = 0; uiIter < dwBytes/sizeof(HMODULE); uiIter++)
		{
			...
			{
				if (fg_StrCmpNoCase(szModName, szModNameArg) == 0)
				{
...
HMODULE fg_GetProcessImageModule(DWORD _PID)
{
	HANDLE hProcess;
	HMODULE _hMod;
	DWORD dwDummy;
	...
	if (!EnumProcessModules(hProcess, &_hMod, sizeof(_hMod), &dwDummy))
		_hMod = NULL;
...
BOOL fg_DejectDLL(DWORD _PID, const WCHAR *_pDLLName, NStr::CStr &_Error)
{
	...
	if (NULL == hmKernel32)
	{
		...
		_Error = CStr::CFormat("Get kernel32 module handle failed: {}") << fg_Win32_GetLastErrorStr(Error);
		return FALSE;
	}
	...
	{
		HRESULT Error = GetLastError();
		_Error = CStr::CFormat("Extracting FreeLibrary failed: {}") << fg_Win32_GetLastErrorStr(Error);
		return FALSE;
	}
	...
	{
		HRESULT Error = GetLastError();
		_Error = CStr::CFormat("Open process failed: {}") << fg_Win32_GetLastErrorStr(Error);
		return FALSE;
	}
	...
	if (NULL == hmDLL)
	{
		_Error = CStr::CFormat("Could not find module in process");
		CloseHandle(hProcess);
		return FALSE;
	}
	...
	while (ExitCode && bRet)
	{
		...
		{
			HRESULT Error = GetLastError();
			_Error = CStr::CFormat("Failed to create remote thread (Dll Unload): {}") << fg_Win32_GetLastErrorStr(Error);
			bRet = FALSE;
		}
...
struct CFileMap
{
	...
	CFileMap(CWStr &_FileName)
		: m_pMemory(nullptr)
	{
		m_File.f_Open(CStr(_FileName), EFileOpen_Read | EFileOpen_RawFileName);
		m_FileMapping = CreateFileMappingW(m_File.f_GetOSFile(), nullptr, PAGE_READONLY, 0, 0, nullptr);

		if (!m_FileMapping)
			DMibErrorFile(CStr::CFormat("Windows returned an error from CreateFileMappingW: {}") << fg_Win32_GetLastErrorStr());
...
//	LoadLibraryW(_pDLLName);
	...
	HMODULE hThisNtDll = g_hNtDll;
	if (!hThisNtDll)
	{
		HRESULT Error = GetLastError();
		fl_ReportError(CStr::CFormat("Get kernel32 module handle failed: {}") << fg_Win32_GetLastErrorStr(Error));
	...
	{
		HRESULT Error = GetLastError();
		fl_ReportError(CStr::CFormat("Extracting LdrLoadDll failed: {}") << fg_Win32_GetLastErrorStr(Error));
		return EInjectDllResult_Failed;
	}
	...
	{
		HRESULT Error = GetLastError();
		fl_ReportError(CStr::CFormat("Could not find NT headers in local ntdll.dll: {}") << fg_Win32_GetLastErrorStr(Error));
		return EInjectDllResult_Failed;
	}
...

	{
		...
		ULONG RetLen = 0;
		if (NLocal::g_fNtQueryInformationProcess && !NLocal::g_fNtQueryInformationProcess(hProcess, ProcessBasicInformation, &BasicInfo, sizeof(BasicInfo), &RetLen) && RetLen == sizeof(BasicInfo))
		{
			SIZE_T ReadBytes = 0;
			...
			if (MemInfo.BaseAddress == MemInfo.AllocationBase && (MemInfo.Type & MEM_IMAGE))
			{
				...
				if (!hRemoteNtDll)
				{
					...
					if (ReadProcessMemory(hProcess, MemInfo.BaseAddress, Temp, 4096, &dwBytes) && dwBytes == 4096)
					{
						...
						if (pRemoteHeaders && fg_MemCmp((uint8 *)pRemoteHeaders, (uint8 *)pThisNtDllHeaders, sizeof(*pThisNtDllHeaders)) == 0)
						{
	...
	CWStr DllDirectories = CFile::fs_GetPath(CWStr(_pDLLName)).f_ReplaceChar('/', '\\');
	...
	size_t StringMemorySize = (fg_StrLen(_pDLLName)+1) * 2;
	...
	{
		HRESULT Error = GetLastError();
		fl_ReportError(CStr::CFormat("Failed to allocate local memory: {}") << fg_Win32_GetLastErrorStr(Error));
		return EInjectDllResult_Failed;
	}
	...
	{
		HRESULT Error = GetLastError();
		fl_ReportError(CStr::CFormat("Failed to allocate remote memory: {}") << fg_Win32_GetLastErrorStr(Error));
		return EInjectDllResult_Failed;
	}
	...
	{
		HRESULT Error = GetLastError();
		fl_ReportError(CStr::CFormat("Failed to write to remote memory: {}") << fg_Win32_GetLastErrorStr(Error));
		return EInjectDllResult_Failed;
	}

	CInjectDllData InjectData;
	InjectData.m_ModuleFileName.Buffer = (ch16 *)lpRemoteMemoryOut;
	InjectData.m_ModuleFileName.Length = (uint16)fg_StrLen(_pDLLName)*sizeof(ch16);
	...
	if (_hThread && bIsCygwin)
	{
		...
		{
			HRESULT Error = GetLastError();
			fl_ReportError(CStr::CFormat("Failed to queue user APC: {}") << fg_Win32_GetLastErrorStr(Error));
			return EInjectDllResult_Failed;
		}
		else
...
#endif
	{
		...
		if (hRemoteThread)
		{
			...
			if (!bRet)
			{
				...
				fl_ReportError
					(
						CStr::CFormat("Failed to join remote thread({}): {} CreateRemote: {}") 
						<< hRemoteThread 
						<< fg_Win32_GetLastErrorStr(Error) 
						<< fg_Win32_GetLastErrorStr(ErrorCreateRemote)
					)
				;
			}
			DWORD ExitCode = 0;
			...
			if (ExitCode != 0)
			{
				...
				fl_ReportError(CStr::CFormat("Error loading library remotely: {} ({}, {})") << fg_Win32_GetLastErrorStr(Result) << ExitCode << Result);
				return EInjectDllResult_Failed;
			}
		}
		else
		{
			HRESULT Error = GetLastError();
			fl_ReportError(CStr::CFormat("Failed to create remote thread (Dll Load): {}") << fg_Win32_GetLastErrorStr(Error));
			return EInjectDllResult_Failed;
		}
	}
...
EInjectDllResult fg_InjectDLL(DWORD _PID, DWORD _ThreadID, const WCHAR *_pDLLName, NStr::CStr &_Error)
{
	...
	{
		HRESULT Error = GetLastError();
		_Error = CStr::CFormat("Open process failed: {}\r\n") << fg_Win32_GetLastErrorStr(Error);
		return EInjectDllResult_Failed;
	}
