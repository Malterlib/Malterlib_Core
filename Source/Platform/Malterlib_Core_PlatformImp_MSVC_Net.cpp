...
// between two queries, so the first only takes its bearings. Whether the sender held that rate
// back is not reported, and the query is made while the sender is bound, so it is taken as so
bool NSys::NNetwork::fg_QueryPathBandwidthDelay(void *_pSocket, umint &o_nBytes, bool &o_bAppLimited)
{
	...
	DWORD nBytes = 0;
	if (WSAIoctl(pSocket->m_Socket, SIO_TCP_INFO, &Version, sizeof(Version), &Info, sizeof(Info), &nBytes, nullptr, nullptr) != 0)
		return false;

	uint64 Stamp = (uint64)NTime::CSystem_Time::fs_GetTimerValue();
	uint64 Frequency = (uint64)NTime::CSystem_Time::fs_TimerFrequency();
	uint64 LastBytesOut = pSocket->m_PathLastBytesOut;
	uint64 LastStamp = pSocket->m_PathLastStamp;
	pSocket->m_PathLastBytesOut = Info.m_BytesOut;
	pSocket->m_PathLastStamp = Stamp;
