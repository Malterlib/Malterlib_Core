...
static void fg_DispatchSocketIoEvent(void *_pToken, NSys::EIoLoopEvent _Events, int _Error)
{
	...
	if (fg_IsSet(_Events, NSys::EIoLoopEvent::mc_ReadClosed))
	{
		...
		// unless the socket error says otherwise. The readable state goes with it: the peer's
		// last payload arrives in the same poll as its close, and a close reported on its own
		// would leave those bytes in the socket for a consumer that reads on the readable state
		// — it drains them and finds the end of the stream, whichever order it takes the two in
...
// The family of an open socket, as the address type the socket carries; none where it cannot say
...
void CWindowsSocketContext::f_StartSocket(CWindowsSocket *_pSocket)
{
	...
	// Decide the send buffer policy now, before the registration exists: the ack-completion
	// option below is set from it, and consumers read the loop's f_SendReleaseIsPrompt answer —
	// which is derived from that option — when they activate completion io, which the
	// registration-applied notification can trigger on the loop's thread the moment the add
	// lands. The SO_SNDBUF option itself is applied later, at the first completion send;
	// fg_SubmitSendVectored explains why.
	//
	...
	// A window set before the socket was started reaches the loop now that there is a
	// registration to hold it
...
namespace
{
	// Whether transfers submitted against this socket will complete on its loop's port: every
	// registration but a readiness-only one is bound there when the loop applies the add. That
	// the add may not have landed yet does not matter: a submission travels through the loop's
	// pending queue and is issued by the loop thread after it has applied the changes queued
	// before it, so the binding precedes the first issued operation, and a binding the kernel
	// refused takes the registration's AFD group with it, which cancels what is pending
	// instead of issuing it. Decided from the options the owner registered with, so the answer
	// is the same whenever it is asked, and no loop thread state is read
...
CWindowsSocket* CWindowsSocketContext::f_InheritHandle2(void *_pOSSocket, NMib::NFunction::TCFunctionMovable<void (::NMib::NNetwork::ENetTCPState _StateAdded)> &&_fOnStateChange)
{
	...
	// The handle comes without the address that would say what it is, and the send window sizes
	// the kernel buffers of a TCP socket while leaving a unix socket's alone. The socket answers
	// for itself
	if (pSocket)
