...
// The registered object signaled, or the removal's cancel found the packet already on its way:
// the two look alike, since the packet carries the status it was armed with, and the
// registration's state tells them apart
...
// Transfers that completed without a packet, reported like any other. Popped from the front, so
// a report that re-posts and re-lists lands behind the rest; bounded per pass so a stream that
// keeps completing at issue cannot starve the hosting queue — leftovers keep the pass from parking
