...
struct alignas(16) CUringIoOp
{
	...
	// Whose notification count this operation is in and, when the send was placed at low
	// occupancy, whose window its lag feeds; the deregistration acknowledgement clears the
	// pointer for notifications that outlive their registration
	CUringRegistration *m_pNotifyRegistration = nullptr;
	uint64 m_ReleaseLagIssueStamp = 0;

	// Receive stream buffer size request
	umint m_nBytes;

	// What the send offered: the in-flight byte accounting adds and removes it, and the exit
	// statistics count a short send against it
...
struct CUringRecvRing
{
	...
	// How far the kernel has filled each bid's buffer: the ring consumes incrementally, so a
	// buffer takes several receives back to back until it is full
