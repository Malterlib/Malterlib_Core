...
CIoLoop_IoUring::~CIoLoop_IoUring()
{
	...
	// and acknowledged by the driving thread before it exited, see ~CIoLoop_Base. No zero copy
	// notification is owed either: the buffer released functor of every send holds its actor's
	// op tracker, the actor's destroy waits for that tracker to drain, and the manager destroys
	// its loops only after every actor is gone
...
umint CIoLoop_IoUring::fp_Iterate(bool _bBlock)
{
	...
	{
		// A ring that cannot be enabled leaves the loop nothing to run its sockets on: this is
		...
		if (!mp_Ring.f_EnableRings())
		{
			...
			DMibPDebugBreak; // Failed to enable the io_uring rings
		}
	}
	...
	// the removal, and the acknowledgement that frees a registration sweeps this queue first
	{
		...
		for (CUringIoOp *pOp : IoOps)
		{
			...
			// A backpressure release rescheduling the stream; moot for one already gone. Its
			// direction flipped when the stream started, so it stays clear of the flip below
	...
	// A placement that met a full ring reaped on the way, and what that dispatched may have
	// enqueued for this thread: the park committed to above is given up then, as it would have
	// been had the reports come earlier. An armed futex wait stays armed; its completion is a
	// wake like any other
	if (bBlock && nReported)
	{
		mp_WakeState.f_Store(0, NAtomic::gc_MemoryOrder_Release);
