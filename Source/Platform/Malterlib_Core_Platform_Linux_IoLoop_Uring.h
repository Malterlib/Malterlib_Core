...
struct CUringRegistration : public NMib::NSys::CIoLoopRegistration
{
	...
	// epochs the loop samples as each zero copy notification arrives; those have their own lock
	...
	// The loop's flush list, see m_bStreamSegmentPending; placed among the pointer sized
	// members so that the flags below pack into one run
	...
	// run and saves nothing. The loop decides on its own thread and the owner asks
	// f_SendReleaseIsPrompt from another, so the result is published after the answer: probed
	// is stored with release once eligible holds the answer, and read with acquire
	NMib::NAtomic::TCAtomic<bool> m_bZeroCopyProbed = false;
	...
	// This registration's zero copy sends whose notification is still outstanding: the
	// occupancy the release lag sample is gated on, so other registrations on the loop cannot
	// starve this one of samples
	umint m_nNotifyPending = 0;
};
...
private:
	...
	DMibListLinkDS_List(CUringRegistration, m_StreamFlushLink) mp_StreamFlushQueue;
	...
	// grows so a long-lived loop cannot wrap into an id still registered, and the counter is
	// wider than the namespace so its exhaustion is a refusal rather than a wrap into an
	// active id
