...
struct CIoSubSystem_Windows : NMib::NSys::CIoSubSystem
{
	...
	// Skipping the port on synchronous success spares a transfer that finishes at issue its
	// packet round trip. On by default; the inline completions are reported a bounded number
	// per pass, so a stream whose data is already waiting cannot report a whole firehose of
	// receives ahead of the jobs that consume them. MalterlibIocpSkipSuccess=0 keeps the packet
	// per transfer for comparison
