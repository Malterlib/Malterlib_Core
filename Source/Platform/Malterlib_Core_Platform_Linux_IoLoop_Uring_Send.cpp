...
// A buffer group id from the free list, else a fresh one; refuses once the u16 namespace is
// spent rather than handing out an id a registered ring still holds
...
bool CIoLoop_IoUring::fp_PublishSend(CUringRegistration *_pRegistration, CUringIoOp *_pOp, umint &_nReported)
{
	...
	// A ring entry's length is the kernel's 32 bit one, so a span wider than that takes several
	// entries
...
void CIoLoop_IoUring::fp_ArmSendBundle(CUringRegistration *_pRegistration)
{
	...
	// The kernel adds MSG_WAITALL to every bundled send itself, since a short send would retire
	// a partly sent entry with the rest of it unsent; asked for here as well so the contract the
	// records depend on is visible where the operation is built
...
void CIoLoop_IoUring::fp_ReleaseNotifyPending(CUringIoOp *_pOp)
{
	...
	// The notification is the release: it leaves the registration's occupancy, and its lag feeds
	// the window's sliding minimum when it was placed at low occupancy, unless the registration
	// is already gone
	...
	-> umint
{
	...
	// The kernel reports the operation's bytes in a signed 32 bit result, and imports at most
	// MAX_RW_COUNT of them, which is that maximum rounded down to a page — 64 KiB pages being
	// the coarsest Linux builds with, so this bound holds whatever the page size. Asking for
	// more would have MSG_WAITALL finish the imported part and report it, which reads here as
	// the short send of a broken connection. A span past the cap is clamped and ends the gather,
	// and the caller offers the remainder again
	...
	// The vectors taken: spans past the vector limit stay the caller's
	return nRequested;
}
