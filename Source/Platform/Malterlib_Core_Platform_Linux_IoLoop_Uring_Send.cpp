	...
	-> umint
{
	...
	// The kernel reports the operation's bytes in a signed 32 bit result, so an operation
	// describes at most that many: a span past the cap is clamped and ends the gather, and the
	// caller offers the remainder again
	constexpr umint c_MaxRequested = umint(TCLimitsInt<int32>::mc_Max);
	umint nVectors = 0;
	umint nRequested = 0;
