...
struct CIoUringCaps
{
	...
	// ring with incremental consumption, needing kernel 6.12 — probed through the send-zc
	// opcode, plus an actual incremental ring registration so a filtered register syscall or
	// an older kernel fails the probe rather than every socket. MalterlibIoUringMultishot=0
	// keeps receives on readiness for comparison
