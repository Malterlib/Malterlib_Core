...
void CIoLoop_IoUring::fp_DispatchCqe(uint64 _UserData, int32 _Res, uint32 _Flags, umint &_nReported)
{
	...
	if (Tag == gc_UringTag_SendOp)
	{
		...
		// A bundle pass imports at most 256 buffers (PEEK_MAX_IMPORT in io_uring/kbuf.c), and
		// while the ring still holds more and the request has not been through poll, the kernel
		// posts the pass with F_MORE and sends on with the same request (io_send_finish in
		// io_uring/net.c). The operation stays armed: only the pass's bytes are covered, and the
		// completion without F_MORE retires it below
		...
		if (pOp->m_bBundle)
		{
			// The passes with F_MORE were covered above. This completion ends the request: its
			// bytes are the last of it, and the ring is re-armed for what remains
		...
		{
			// MSG_WAITALL has the kernel finish the buffer; one it hands back short is the
			// connection going away under it — or a cancellation that caught it part way, which
			// the kernel reports as the bytes placed rather than as cancelled — and the contract
			// is that Done means all of the scheduled bytes, so it is reported as the failure it
			// is. The bytes that did leave travel with it, so a teardown's truncation point stays
			// known, as the bundle path's records do
		...
		if (_Flags & gc_IoUringCqe_FMore)
		{
			// Only a send with at most one of its registration's notifications ahead of it samples
			// the release lag: one behind a standing queue measures the queue, and a window sized
			// by its own occupancy only grows the occupancy. The loop's other registrations do not
			// count, since their queues say nothing about this path
	...
	if (Tag == gc_UringTag_RecvStream)
	{
		...
		if (_Res > 0)
		{
			...
			// the last is filled to the end, and a partially filled tail stays with the
			// kernel at its offset — so the walk below attributes the bytes over the ring's
			// own publish-order queue. Retired buffers leave the ring
			...
			while (nRemaining)
			{
				...
				// A buffer filled to its end is done with the kernel; a partially filled one
				// stays at its offset for the next receive. Deliberately not consulting the
				// buf-more flag: on a bundled completion it describes only the NAMED (first)
				// bid, and the kernel retains a partially filled tail regardless of it —
				// probed byte-exact against the target kernel, including partial tails
				// surviving multishot termination and re-arm — so the arithmetic here is the
				// authoritative retirement rule, and a kernel that ever broke it would trip
				// the publish-order check above rather than corrupt
