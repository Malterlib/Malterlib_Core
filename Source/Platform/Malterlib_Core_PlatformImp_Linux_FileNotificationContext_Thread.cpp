...
void CFileChangeNotificationContext::CNotificationThread::f_ReadEvents()
{
	...
	TCMap<uint32, TCSharedPointer<CWatch>> PendingRenames;
	TCMap<uint32, bool> PendingRenameIsDirectory;
	while (true)
	{
		...
		while (ReadResult > Processed)
		{
			...
			else if (pEvent->mask & IN_MOVED_FROM)
			{
				...
				{
					PendingRenameIsDirectory[pEvent->cookie] = *pIsDir;
					Watch.m_ChildFiles.f_Remove(EventPath);
				}
				auto *pChild = Watch.f_GetChild(EventPath);
				if (pChild)
					PendingRenames[pEvent->cookie] = fg_Explicit(pChild);
			}
			else if (pEvent->mask & IN_MOVED_TO)
			{
				if (auto *pIsDir = PendingRenameIsDirectory.f_FindEqual(pEvent->cookie))
					Watch.m_ChildFiles[EventPath] = *pIsDir;
				else
					Watch.m_ChildFiles[EventPath] = CFile::fs_FileExists(FullEventPath, EFileAttrib_Directory);
				
				auto pRenameFrom = PendingRenames.f_FindEqual(pEvent->cookie);
				if (pRenameFrom)
				{
					(*pRenameFrom)->f_SetParent(&Watch, FullEventPath);
	...
	for (auto &pPendingRename : PendingRenames)
	{
		pPendingRename->f_SetParent(nullptr, CStr());
	}
	...
	for (auto &Context : NotificationContexts)
	{
		...
		for (auto &Rename : pNotification->m_PendingRenames)
			pNotification->f_OnRemovedFromRename(Context, Rename);
		pNotification->m_PendingRenames.f_Clear();
