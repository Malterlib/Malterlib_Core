	...
	-> umint
{
	...
	// Read before the queue trip: the loop can issue, complete and free the operation the moment
	// it is queued when the submit comes from another thread
...
// A send on a socket that finishes sends only at the acknowledgement is reported here, at
// issue, whatever the kernel made of it: taken, pended or refused. The consumers count the
// results against the front of their queues, so every result has to reach them in issue order
// — which is submission order, what the accounting would see from a buffered send — and a
// refusal held back to the FIFO while the next send's acceptance was reported at once would
// credit that send's bytes to the refused one. The FIFO entry that follows runs the release
// alone. An overlapped stream send transfers all of its bytes unless the connection fails, and
// a failure after the acceptance reaches the consumer through the socket, as it does for a
// buffered send whose bytes the kernel took
...
void CIoLoop_Iocp::fp_IssueSend(CIocpRegistration *_pRegistration, CIocpSendOp *_pOp, umint &_nReported)
{
	...
	// Taken before anything else runs, as in fp_PostRecv: the trace below leaves a last error
	// of its own behind, and a pending send read back as a failure would release buffers the
	// kernel is still sending from
	...
	// Refused at issue: nothing reached the kernel and no packet follows. The completion is owed
	// through the inline list, which reports it in submission order behind the sends ahead of
	// it; on an acknowledgement-completing socket the result itself is told now, in issue order
	// with the acceptances, and the inline entry runs the release
	_pOp->m_bCompleted = true;
