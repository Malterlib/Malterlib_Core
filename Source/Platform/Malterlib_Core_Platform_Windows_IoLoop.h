...
struct CIocpRegistration : public NMib::NSys::CIoLoopRegistration
{
	...
	// Whether the socket's sends complete only at the peer's acknowledgement, from the
	...
	// The adaptive window the consumer asks about. The asker owns it, except the release lag
	// epochs the loop samples as each acknowledgement-completing send reports; those have their
	// own lock
	...
	// The loop's flush list, see m_bStreamSegmentPending, and its inline completion list, in
	// which the registration holds one obligation on the count while listed so the record
	// outlives its entry. Placed among the pointer sized members so that the flags below pack
	// into one run
	DMibListLinkDS_Link(CIocpRegistration, m_StreamFlushLink);
	DMibListLinkDS_Link(CIocpRegistration, m_InlineLink);

	// The flags together, behind the wider members, so they pack. The poll's: armed, a cancel
	// asked for, and the readiness reports owed, one-shot, cleared when the poll reports that
	// direction
	...
	// A merged, undelivered receive segment stands; the registration is in the loop's flush
	// list exactly then
	...
	// must not be submitted. The loop thread's alone: the owner decides whether it may submit
	// from the registration options, see fg_CompletionPortIsOwningLoops
...
private:
	...
	// registration is listed at most once and pinned while listed. Popped from the front, so a
	// report that re-lists one lands behind what is still to come
	DMibListLinkDS_List(CIocpRegistration, m_InlineLink) mp_InlineCompletions;

	// Registrations holding a merged, undelivered receive segment, in first-staged order;
	// each listed once, and unlinked as its segment is delivered or dropped
	DMibListLinkDS_List(CIocpRegistration, m_StreamFlushLink) mp_StreamFlushQueue;
