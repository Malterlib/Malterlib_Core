...
namespace NMib::NSys
{
	...
	struct CIoStreamBackpressure
	{
		...
		// The most delivered bytes the consumer keeps referenced before it can release any of
		// them — a message it assembles from views, at its largest. Set before the stream starts;
		// the backend raises the limit and the resume threshold for the partly used buffers such
		// a hold pins on top, which is its to know: how large its buffers are and how many are
		// being filled at once. Zero states no such hold
		umint m_nConsumerHoldBytes = 0;
