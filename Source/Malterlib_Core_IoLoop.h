...
namespace NMib::NSys
{
	...
	// The errors the loops report are the platform's socket errors: Winsock's on Windows, where
	// the C runtime's errno values are a different numbering, and errno elsewhere. A consumer
	// reporting a connection reset of its own names it in that same space
...
// ride the same loop
	...
	struct ICIoLoop : public ICThreadIoLoop
	{
		...
		// Answers the bytes it scheduled: every one of them leaves before the completion reports
		// Done, and a transfer the kernel ends short reports Error instead. The count can be less
		// than offered when a span or the span count is beyond what one operation describes; the
		// remainder is the caller's to offer again. 0 is a refusal, and terminal
		...
		//
		// A buffer is charged its whole capacity, and every backend fills a buffer to its end
		// before taking the next. A consumer that keeps the segments of one message until it
		// is complete states the most it holds that way in m_nConsumerHoldBytes; the backend
		// knows how large its buffers are and how many are being filled at once, so it raises
		// the limit and the resume threshold for the partly used buffers such a hold pins on
		// top of its own bytes, and a stream parked with only the hold outstanding still
		// resumes once everything else is released
