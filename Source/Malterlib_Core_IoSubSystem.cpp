...
namespace NMib::NSys
{
	CIoSubSystem::CIoSubSystem()
	{
		...
		// Direction-specific overrides take precedence over the shared override.
		...
		mp_nReceiveWindowBytesOverride = fsp_EnvCount("MalterlibReceiveWindow", 0, 1, umint(fg_Min(uint64(1) << 40, uint64(TCLimitsInt<umint>::mc_Max))));
	...
	// Thread-safe; duplicate registrations print once, and only registered areas are reported.
	...
	// Called during module exit and destruction; the once flag prevents duplicate reports.
